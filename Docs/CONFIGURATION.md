...
`configuration.signatures.defaultLineWidth` sets the stroke width used when drawing new signatures.
`configuration.features.allowsColorPicker` enables the system color picker in text and brush color inspectors in addition to the configured palette. Signature creation uses the configured signature palette only.
